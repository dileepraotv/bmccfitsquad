...
async def format_activity_notification(activity: dict, athlete_name: str) -> str:
    """Format a raw Strava activity dict into a BMCC Telegram notification.

    The ``activity`` dict should contain Strava API field names.  It may come
    directly from the Strava API response or be assembled from an ORM Activity
    instance by :func:`send_activity_notification`.

    Args:
        activity:     Dict with Strava activity fields (see module docstring).
    ...
    # ------------------------------------------------------------------
    # Section 4 — Elevation & optional heart-rate
    # ------------------------------------------------------------------
    ...
    lines += [
        "",
        f"Elevation Gain: {int(elevation_m)} meters",
    ]
